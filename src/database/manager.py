...
class DatabaseManager:
    ...
    Database Schema:
    - speakers table: id, name, speaker_id, audio_path, register_date, deleted
    - embeddings table: id, speaker_id, model_name, embedding_path
    """
    ...
    def __init__(self, data_dir: str = "data"):
        ...
        self.old_metadata_file = self.data_dir / "speakers.json"
    ...
    def _migrate_from_json(self):
        ...
        if not self.old_metadata_file.exists():
            return
        ...
        import json
        try:
            with open(self.old_metadata_file, 'r', encoding='utf-8') as f:
            ...
            print(f"📦 Migrating {len(speakers)} speakers from JSON to SQLite...")
            
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            
            for speaker_id, speaker_data in speakers.items():
                # Migrate speaker entry
                cursor.execute('''
                    INSERT OR IGNORE INTO speakers 
                    (speaker_id, name, audio_path, register_date, deleted)
                    VALUES (?, ?, ?, ?, ?)
                ''', (
                    speaker_id,
                    speaker_data.get('name', 'Unknown'),
                    speaker_data.get('audio_path', 'data/audio/' + speaker_id + '.wav'),
                    speaker_data.get('register_date', ''),
                    0
                ))
                
                # Migrate embeddings
                embeddings = speaker_data.get('embeddings', {})
                for model_name, emb_path in embeddings.items():
                    if emb_path:
                        cursor.execute('''
                            INSERT OR IGNORE INTO embeddings 
                            (speaker_id, model_name, embedding_path)
                            VALUES (?, ?, ?)
                        ''', (speaker_id, model_name, emb_path))
            
            conn.commit()
            conn.close()
            
            print(f"✅ Migration complete: {len(speakers)} speakers imported")
    ...
    def _get_next_speaker_id(self) -> str:
        ...
        # Get MAX from ALL speakers (including deleted) to avoid ID conflicts
        cursor.execute('SELECT MAX(CAST(SUBSTR(speaker_id, 9) AS INTEGER)) FROM speakers')
    ...
    def list_speakers(self) -> List[Dict]:
        ...
        Returns:
            List of speaker dictionaries with 'id' and 'name' keys
        """
        conn = sqlite3.connect(self.db_path)
        ...
        speakers = [{'id': row['speaker_id'], 'name': row['name']} for row in cursor.fetchall()]
        conn.close()
    ...
    def metadata(self):
        ...
        for speaker in speakers_list:
            speakers_dict[speaker['id']] = {
                'name': speaker['name'],
                'id': speaker['id'],
                'embeddings': {
                    'ecapa': f"embeddings/{speaker['id']}_ecapa.npy",
                    'titanet': f"embeddings/{speaker['id']}_titanet.npy",
                    'wavlm': f"embeddings/{speaker['id']}_wavlm.npy"
                }
            }
